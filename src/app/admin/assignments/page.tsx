...
const AssignmentsPage = async () => {
  ...
  const [assignmentsRes, teachersRes, offeringsRes, qualificationsRes] = await Promise.all([
  ...
  const { data: teachers, error: teachersError } = teachersRes;
  const { data: classOfferings, error: offeringsError } = offeringsRes;
  const { data: qualifications, error: qualificationsError } = qualificationsRes;

  if (assignmentsError || teachersError || offeringsError || qualificationsError) {
    console.error({ assignmentsError, teachersError, offeringsError, qualificationsError });
    return (
      <Container>
        <Text color="red">Failed to load necessary data for assignments page.</Text>
      </Container>
    );
  }
  ...
  return (
    <AssignmentsClientComponent
      initialAssignments={assignments as unknown as TeachingAssignment[]}
      teachers={teachers as unknown as Teacher[]}
      classOfferings={classOfferings as unknown as ClassOffering[]}
      qualifications={qualifications as unknown as TeacherQualification[]}
    />
  );
};
